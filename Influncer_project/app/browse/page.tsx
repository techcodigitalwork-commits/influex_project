...
export default function BrowsePage() {
  ...
  const fetchAllCreators = async (): Promise<void> => {
    ...
    try {
      ...
      const cleaned = list.map((c: any) => { const { phone, ...rest } = c; return rest; });
      setCreators(cleaned);
    ...
    try {
      ...
      try {
        ...
        if (!campText.startsWith("<!")) {
          const camps = (JSON.parse(campText).data || JSON.parse(campText).campaigns || []);
  ...
  const getCats = (c: Creator): string[] => { const cats = c.categories || []; return (Array.isArray(cats) ? cats : [cats as string]).filter(Boolean); };
  ...
  const catEmoji: Record<string, string> = { fashion:"👗", beauty:"💄", fitness:"💪", food:"🍕", travel:"✈️", tech:"💻", lifestyle:"🌟", gaming:"🎮", music:"🎵", sports:"⚽", education:"📚", comedy:"😂", dance:"💃", photography:"📷", art:"🎨" };
      ...
      <div className="browse">
        ...
        {loading ? (
          ...
          <div className="browse-empty">
            ...
            <p className="browse-empty-sub">{creators.length === 0 ? "Creators will appear here once they register." : "Try clearing filters"}</p>
          </div>
        ) : (
                ...
                return (
                  <div key={id} className={\`cc \${isBlurred ? "cc-blurred" : ""}\`}
                    ...
                    <div className="cc-top">
                      {img ? <img src={img} alt={name} className="cc-av" /> : <div className="cc-av-ph">{name.charAt(0).toUpperCase()}</div>}
                    ...
                    <div className="cc-body">
                      <p className="cc-bio">{isBlurred ? "Upgrade to view this creator's profile..." : (bio || "Creator available for brand collaborations.")}</p>
                      ...
                      <div className="cc-actions">
                        {isBlurred ? (
                          <button className="cc-btn cc-btn-upgrade" onClick={() => router.push("/upgrade")}>🔓 Unlock — Upgrade Plan</button>
                        ) : status === "accepted" ? (
                          <><button className="cc-btn cc-btn-accepted" disabled>✓ Connected</button><button className="cc-btn-chat" onClick={() => router.push(\`/messages?with=\${id}\`)} title="Chat">💬</button></>
                        ) : status === "pending" ? (
                          <button className="cc-btn cc-btn-pending" disabled>⏳ Connecting...</button>
                        ) : (
                          <button className="cc-btn cc-btn-connect" onClick={() => handleConnect(id)} disabled={isConnecting}>{isConnecting ? "Connecting..." : "🤝 Connect"}</button>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            {filtered.length > FREE_LIMIT && (
              <div className="upgrade-wall">
                <div className="upgrade-wall-inner">
                  ...
                  <p className="upgrade-wall-sub">Upgrade to Pro to unlock all creators and connect without limits</p>
