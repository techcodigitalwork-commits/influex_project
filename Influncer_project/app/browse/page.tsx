...
export default function BrowsePage() {
  ...
  const [viewProfile, setViewProfile] = useState<Creator | null>(null);
  ...
  useEffect(() => {
    ...
    setConnectStatus(JSON.parse(localStorage.getItem("connectStatus") || "{}"));
  }, []);

  useEffect(() => {
    if (!token) return;
    fetchAllCreators();
    const interval = setInterval(checkAcceptedConnections, 10000);
  ...
  const checkAcceptedConnections = async () => {
    try {
      ...
      const convs = JSON.parse(text).data || JSON.parse(text).conversations || [];
      const saved: Record<string, ConnectStatus> = JSON.parse(localStorage.getItem("connectStatus") || "{}");
      let changed = false;
      convs.forEach((c: any) => {
        (c.participants || []).forEach((p: any) => {
          const pid = typeof p === "object" ? (p._id || p.id) : p;
          if (pid && pid !== myId && saved[pid] === "pending") { saved[pid] = "accepted"; changed = true; }
        });
      });
      if (changed) { setConnectStatus({ ...saved }); localStorage.setItem("connectStatus", JSON.stringify(saved)); }
    } catch { }
  };
  ...
  const fetchAllCreators = async () => {
    ...
    try {
      ...
      const list: Creator[] = (JSON.parse(text).data || []).map((c: any) => { const { phone, ...r } = c; return r; });
      setCreators(list);
  ...
  const handleConnect = async (creatorId: string) => {
    ...
    try {
      await fetch(`${API}/notification/create`, {
        ...
        body: JSON.stringify({ user: creatorId, message: "A brand wants to connect with you!", type: "new_message", link: "/messages" }),
      });
      let campaignId = "";
      try {
        const cr = await fetch(`${API}/campaigns/my`, { headers: { Authorization: `Bearer ${token}` } });
        const ct = await cr.text();
        if (!ct.startsWith("<!")) { const camps = JSON.parse(ct).data || JSON.parse(ct).campaigns || []; if (camps[0]) campaignId = camps[0]._id; }
      } catch { }
      const convRes = await fetch(`${API}/conversations/create`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ participantId: creatorId, ...(campaignId ? { campaignId } : {}) }),
      });
      const newStatus: ConnectStatus = convRes.ok ? "accepted" : "pending";
  ...
  const gFollowers = (c: Creator) => {
    const f = c.followers; if (!f || f === "0" || f === 0) return "";
    const n = Number(f);
  ...
  const em: Record<string, string> = { fashion:"👗",beauty:"💄",fitness:"💪",food:"🍕",travel:"✈️",tech:"💻",lifestyle:"🌟",gaming:"🎮",music:"🎵",sports:"⚽",education:"📚",comedy:"😂",dance:"💃",photography:"📷",art:"🎨" };
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .bgrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:18px;padding:0 40px 48px}
        ...
        .cbtn{flex:1;padding:11px;border-radius:11px;font-size:13px;font-weight:700;font-family:'Plus Jakarta Sans',sans-serif;border:none;cursor:pointer;transition:all 0.2s}
        .cbtn-connect{background:linear-gradient(135deg,#4f46e5,#7c3aed);color:#fff;box-shadow:0 2px 10px rgba(79,70,229,0.3)}
        .cbtn-connect:hover:not(:disabled){transform:translateY(-1px);box-shadow:0 4px 16px rgba(79,70,229,0.4)}
        .cbtn-connect:disabled{opacity:0.6;cursor:not-allowed}
        .cbtn-pending{background:#fffbeb;color:#d97706;border:1.5px solid #fde68a !important;cursor:default}
        .cbtn-profile{background:#f5f3ff;color:#7c3aed;border:1.5px solid #ddd6fe !important;cursor:pointer}
      ...
      {/* PROFILE MODAL */}
      {viewProfile && (
        <div className="mo" onClick={() => setViewProfile(null)}>
          <div className="mo-box" onClick={e => e.stopPropagation()}>
            <button className="mo-close" onClick={() => setViewProfile(null)}>✕</button>
            <div className="mo-banner" />
            <div className="mo-avwrap">
              {gImg(viewProfile)
                ? <img src={gImg(viewProfile)!} alt={gName(viewProfile)} className="mo-av" />
                : <div className="mo-avph">{gName(viewProfile).charAt(0).toUpperCase()}</div>
              }
            </div>
            <div className="mo-body">
              <div className="mo-name">{gName(viewProfile)}</div>
              <div className="mo-tags">
                {gCats(viewProfile)[0] && <span className="mo-tag mo-tag-cat">{em[gCats(viewProfile)[0]?.toLowerCase()] || "✨"} {cap(gCats(viewProfile)[0])}</span>}
                {gCity(viewProfile) && <span className="mo-tag mo-tag-city">📍 {cap(gCity(viewProfile))}</span>}
              </div>
              {gBio(viewProfile) && <div className="mo-bio">{gBio(viewProfile)}</div>}
              <div className="mo-stats">
                <div className="mo-stat"><div className="mo-sval">{gFollowers(viewProfile) || "—"}</div><div className="mo-slbl">Followers</div></div>
                <div className="mo-stat"><div className="mo-sval">{gCats(viewProfile).length || "—"}</div><div className="mo-slbl">Niches</div></div>
                <div className="mo-stat"><div className="mo-sval">{gCity(viewProfile) ? cap(gCity(viewProfile)) : "—"}</div><div className="mo-slbl">City</div></div>
              </div>
              <div className="mo-info">
                {gCity(viewProfile) && <div className="mo-row"><span>📍</span><span>{cap(gCity(viewProfile))}</span></div>}
                {gCats(viewProfile).length > 0 && <div className="mo-row"><span>🎯</span><span>{gCats(viewProfile).map(cap).join(", ")}</span></div>}
                {viewProfile.platform && (
                  <div className="mo-row">
                    <span>🔗</span>
                    <a href={viewProfile.platform} target="_blank" rel="noreferrer" style={{ color: "#4f46e5", textDecoration: "none", wordBreak: "break-all" }}>{viewProfile.platform}</a>
                  </div>
                )}
                {viewProfile.createdAt && (
                  <div className="mo-row">
                    <span>📅</span>
                    <span>Joined {new Date(viewProfile.createdAt).toLocaleDateString("en-IN", { year: "numeric", month: "long" })}</span>
                  </div>
                )}
              </div>
              <div className="mo-acts">
                <button className="mo-btn mo-chat" onClick={() => { setViewProfile(null); router.push(`/messages?with=${viewProfile._id}`); }}>💬 Open Chat</button>
                <button className="mo-btn mo-cls" onClick={() => setViewProfile(null)}>Close</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="browse">
        ...
        {loading ? (
          ...
          <div className="bempty">
            ...
            <p className="bempty-sub">{creators.length === 0 ? "Creators will appear here once they register." : "Try clearing filters"}</p>
          </div>
        ) : (
          <div className="bgrid">
            {filtered.map((creator) => {
              ...
              const i = gImg(creator);
              const n = gName(creator);
              return (
                <div key={id} className="cc">
                  {status === "accepted" && <div className="cc-badge cc-badge-ok">✓ Connected</div>}
                  {status === "pending"  && <div className="cc-badge cc-badge-pend">⏳ Request Sent</div>}
                  <div className="cc-top">
                    {i ? <img src={i} alt={n} className="cc-av" /> : <div className="cc-avph">{n.charAt(0).toUpperCase()}</div>}
                  ...
                  <div className="cc-body">
                    ...
                    <div className="cc-acts">
                      {status === "accepted" ? (
                        <>
                          <button className="cbtn cbtn-profile" onClick={() => setViewProfile(creator)}>👤 View Profile</button>
                          <button className="cbtn-chat" onClick={() => router.push(`/messages?with=${id}`)} title="Chat">💬</button>
                        </>
                      ) : status === "pending" ? (
                        <button className="cbtn cbtn-pending" disabled>⏳ Request Sent</button>
                      ) : (
                        <button className="cbtn cbtn-connect" onClick={() => handleConnect(id)} disabled={isConn}>
