...
export default function BrowsePage() {
  ...
  const fetchCreators = async (): Promise<void> => {
    try {
      ...
      // ✅ Source 0: Direct creators list — agar backend pe endpoint hai
      let directLoaded = false;
      try {
        const directRes = await fetch(`${API}/users/creators`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const directText = await directRes.text();
        if (!directText.startsWith("<!") && directRes.ok) {
          const directData = JSON.parse(directText);
          const list = directData.data || directData.users || [];
          if (list.length > 0) {
            console.log("✅ Direct creators loaded:", list.length);
            setCreators(list);
            directLoaded = true;

            // Filters bhi set karo
            const ns = new Set<string>();
            const cs = new Set<string>();
            list.forEach((c: Creator) => {
              const cats = c.categories || [];
              const arr: string[] = Array.isArray(cats) ? cats : [cats as string];
              arr.forEach((cat: string) => { if (cat?.trim()) ns.add(cat.toLowerCase().trim()); });
              const city = c.city || c.location || "";
              if (city?.trim()) cs.add(city.toLowerCase().trim());
            });
            setNiches([...ns].filter(Boolean));
            setCities([...cs].filter(Boolean));
            setLoading(false);
            return; // Direct endpoint kaam kiya — baaki sources skip karo
          }
        }
      } catch { /* endpoint nahi hai — fallback use karo */ }

      // Source 1: Brand campaigns applications (fallback)
      try {
        ...
        if (!campText.startsWith("<!")) {
          ...
          for (const camp of campaigns) {
            try {
              ...
              apps.forEach((a: any) => {
                // ✅ influencerId object ho sakta hai populated form mein
                let id = "";
                if (typeof a.influencerId === "string") id = a.influencerId;
                else if (typeof a.influencerId === "object" && a.influencerId?._id) id = a.influencerId._id;
                else if (typeof a.influencer === "string") id = a.influencer;
                else if (typeof a.influencer === "object" && a.influencer?._id) id = a.influencer._id;
                else if (typeof a.userId === "string") id = a.userId;
                else if (typeof a.user?._id === "string") id = a.user._id;
                if (id && id.length === 24) creatorIds.add(id);
              });
            } catch { continue; }
          }
        }
      ...
      try {
        ...
        if (!nt.startsWith("<!")) {
          ...
          (nd.data || nd.notifications || []).forEach((n: any) => {
            const id = n.sender || n.from?._id || n.fromId || n.senderId || n.applicantId;
            // ✅ Sirf valid string IDs
            if (id && typeof id === "string" && id.length === 24) creatorIds.add(id);
          });
        }
      } catch { /* skip */ }
        ...
        if (!ct.startsWith("<!")) {
          ...
          (cd.data || cd.conversations || []).forEach((c: any) => {
            (c.participants || []).forEach((p: any) => {
              // ✅ p object ho sakta hai ya string — dono handle karo
              let id = "";
              if (typeof p === "string") id = p;
              else if (typeof p === "object" && p !== null) id = p._id || p.id || "";
              // ✅ Sirf valid MongoDB IDs add karo (24 char hex)
              if (id && typeof id === "string" && id.length === 24 && id !== myId) {
                creatorIds.add(id);
              }
            });
          });
