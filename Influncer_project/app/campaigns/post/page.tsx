...
export default function PostCampaignPage() {
  ...
  useEffect(() => {
    ...
    // Fast from localStorage
    const localCoins = parsed.coins ?? parsed.bits ?? FREE_COINS;
    setCoins(localCoins);
    setIsSubscribed(parsed.isSubscribed ?? false);

    // Show modal immediately if not enough coins (before API)
    if (!(parsed.isSubscribed) && localCoins < COINS_PER_CAM) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Coin gate
    if (!isSubscribed && coins < COINS_PER_CAM) {
    ...
    if (formData.roles.length === 0) {
      showToast("Select at least one role", "error"); return;
    }
    ...
    try {
      ...
      if (!isSubscribed) {
        ...
        const stored  = localStorage.getItem("cb_user");
        const parsed  = JSON.parse(stored || "{}");
        localStorage.setItem("cb_user", JSON.stringify({ ...parsed, coins: newCoins }));
        // Auto-show modal if now empty
    ...
    const options = {
      ...
      handler: async function (response: any) {
        ...
        if (aData.success) {
          ...
          showToast(`🎉 ${planName} activated! Ab unlimited campaigns post karo!`, "success");
        } else {
  ...
  const handleSubscribe = async (planId: string, planName: string) => {
    ...
    if (!t) { showToast("Session expired. Login again.", "error"); return; }
    ...
    <>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .pcp-input { width:100%; padding:12px 14px; border:1.5px solid #e5e7eb; border-radius:12px; font-size:14px; font-family:'DM Sans',sans-serif; color:#111; background:#fff; outline:none; transition:border-color 0.2s,box-shadow 0.2s; }
        ...
        .coin-modal-close { position:absolute; top:14px; right:16px; background:none; border:none; font-size:20px; cursor:pointer; color:#aaa; line-height:1; padding:4px; }
        ...
        .coin-modal-plans { display:flex; flex-direction:column; gap:10px; margin-bottom:14px; }
        .coin-plan-btn { padding:14px 20px; border-radius:14px; border:none; font-size:15px; font-weight:600; font-family:'DM Sans',sans-serif; cursor:pointer; transition:all 0.2s; display:flex; align-items:center; justify-content:space-between; }
        .coin-plan-btn.pro { background:linear-gradient(135deg,#4f46e5,#7c3aed); color:#fff; box-shadow:0 4px 16px rgba(79,70,229,0.3); }
        .coin-plan-btn.pro:hover { box-shadow:0 6px 24px rgba(79,70,229,0.4); transform:translateY(-1px); }
        .coin-plan-btn:disabled { opacity:0.65; cursor:not-allowed; transform:none !important; }
        ...
        <div className="coin-modal-overlay">
          <div className="coin-modal">
            ...
            <div className="coin-modal-title">
              {coins <= 0 ? "Coins Khatam Ho Gaye!" : "Coins Kam Hain!"}
            </div>
            <div className="coin-modal-sub">
              {coins <= 0
                ? `Saare ${FREE_COINS} free coins use ho gaye. Pro plan leke unlimited campaigns post karo.`
            ...
            <div className="coin-modal-prog-wrap">
              ...
              <div className="coin-modal-prog">
                <div className="coin-modal-prog-fill" style={{
                  width: `${100 - coinsPercent}%`,
                  background: coins <= 0 ? "#ef4444" : "#f59e0b",
                }} />
              </div>
            </div>
            <div className="coin-modal-plans">
              <button
                className="coin-plan-btn pro"
          ...
          <div className="pcp-card">
            ...
            >
              ...
              <div className="pcp-coin-info">
                <div className="pcp-coin-label">
                  {isSubscribed ? "Unlimited Coins (Pro)" : `Coins — ${COINS_PER_CAM} per campaign`}
                </div>
                {!isSubscribed && (
                ...
                <div className="pcp-coin-count" style={{ color: isSubscribed ? "#15803d" : usageColor }}>
                  {isSubscribed
                    ...
                    : coinsEmpty
                      ? "⚠️ Coins nahi hain — upgrade karo"
            ...
            {coinsEmpty ? (
              <div className="pcp-blocked">
                ...
                <p className="pcp-blocked-sub">
                  Free plan mein <strong>{FREE_COINS / COINS_PER_CAM} campaigns</strong> allowed hain ({COINS_PER_CAM} coins each).<br />
                  Pro upgrade karo aur unlimited campaigns post karo.
                </p>
                <button className="pcp-blocked-btn" onClick={() => setShowCoinModal(true)}>
                  ⚡ Upgrade to Pro →
                </button>
              </div>
            ) : (
