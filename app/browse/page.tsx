...
// // "pending"  → Request sent, waiting for creator to accept
// // "accepted" → Creator accepted → Show View Profile + Chat
// type ConnectStatus = "none" | "pending" | "accepted";
...
//     if (!token) return;
//     fetchAllCreators();
//     // Poll every 8 seconds to check if creator accepted
//     const interval = setInterval(checkAcceptedConnections, 8000);
//     return () => clearInterval(interval);
//   }, [token]);

//   // ─────────────────────────────────────────────────────────────
//   // CHECK: Has creator accepted?
//   // Logic: If a conversation exists with that creator AND
//   //        the conversation has been "accepted" / is active,
//   //        then move status from "pending" → "accepted"
//   //
//   // We check the conversation list from backend.
//   // If the creator is a participant in any active conversation → accepted.
//   // ─────────────────────────────────────────────────────────────
//   const checkAcceptedConnections = async () => {
//     try {
//       const saved: Record<string, ConnectStatus> = JSON.parse(
...
//       // Only check if there are any pending requests
//       const pendingIds = Object.keys(saved).filter((k) => saved[k] === "pending");
//       if (pendingIds.length === 0) return;
...
//       convs.forEach((conv: any) => {
//         // Only count conversation as "accepted" if it has a specific accepted status
//         // OR if the conversation was created by the other side (creator initiated)
//         const isAccepted =
...
//       // Fallback: if backend doesn't return status field,
//       // check if conversation has messages (means creator replied = accepted)
//       if (!changed) {
//         convs.forEach((conv: any) => {
//           const hasMessages =
//             (conv.lastMessage && conv.lastMessage !== "") ||
...
//   // ─────────────────────────────────────────────────────────────
//   // CONNECT: Always sets status to "pending"
//   // Never sets "accepted" here — only checkAcceptedConnections does that
//   // ─────────────────────────────────────────────────────────────
//   const handleConnect = async (creatorProfileId: string) => {
//     const status = connectStatus[creatorProfileId] || "none";
//     if (status === "pending" || status === "accepted") return;
...
//       // backend needs profile.user = actual user ID
//       const creator = creators.find(c => c._id === creatorProfileId);
//       const creatorUserId = (creator as any)?.user || creatorProfileId;

//       // campaignId is REQUIRED by backend for conversation/create
//       let campaignId = "";
//       try {
//         const cr = await fetch(`${API}/campaigns/my`, { headers: { Authorization: `Bearer ${token}` } });
//         const ct = await cr.text();
...
//       if (!campaignId) {
//         // No campaign found — set pending anyway
//         const updated = { ...connectStatus, [creatorProfileId]: "pending" as ConnectStatus };
//         setConnectStatus(updated);
//         localStorage.setItem("connectStatus_" + myId, JSON.stringify(updated));
//         return;
...
//       // Step 2: Notification — NO sender field (not in schema), type must be "new_message"
//       await fetch(`${API}/notification/create`, {
//         method: "POST",
//         headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
//         body: JSON.stringify({
...
//         /* Pending button — yellow, non-clickable */
//         .cbtn-pending{flex:1;padding:11px;border-radius:11px;font-size:13px;font-weight:700;font-family:'Plus Jakarta Sans',sans-serif;border:1.5px solid #fde68a;cursor:default;background:#fffbeb;color:#d97706}
...
//         /* Pending info box */
//         .cc-pending-info{background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:10px 12px;margin-bottom:12px;font-size:12px;color:#92400e;line-height:1.5;text-align:center}
...
//       {/* PROFILE MODAL — only visible when accepted */}
//       {viewProfile && (
...
//               <div className="mo-tags">
//                 {gCats(viewProfile)[0] && (
//                   <span className="mo-tag mo-tag-cat">{em[gCats(viewProfile)[0]?.toLowerCase()] || "✨"} {cap(gCats(viewProfile)[0])}</span>
//                 )}
//                 {gCity(viewProfile) && (
//                   <span className="mo-tag mo-tag-city">📍 {cap(gCity(viewProfile))}</span>
//                 )}
//               </div>
//               {gBio(viewProfile) && <div className="mo-bio">{gBio(viewProfile)}</div>}
//               <div className="mo-stats">
//                 <div className="mo-stat">
//                   <div className="mo-sval">{gFollowers(viewProfile) || "—"}</div>
//                   <div className="mo-slbl">Followers</div>
//                 </div>
//                 <div className="mo-stat">
//                   <div className="mo-sval">{gCats(viewProfile).length || "—"}</div>
//                   <div className="mo-slbl">Niches</div>
//                 </div>
//                 <div className="mo-stat">
//                   <div className="mo-sval">{gCity(viewProfile) ? cap(gCity(viewProfile)) : "—"}</div>
//                   <div className="mo-slbl">City</div>
//                 </div>
//               </div>
//               <div className="mo-info">
//                 {gCity(viewProfile) && (
//                   <div className="mo-row"><span>📍</span><span>{cap(gCity(viewProfile))}</span></div>
//                 )}
//                 {gCats(viewProfile).length > 0 && (
//                   <div className="mo-row"><span>🎯</span><span>{gCats(viewProfile).map(cap).join(", ")}</span></div>
//                 )}
//                 {viewProfile.platform && (
//                   <div className="mo-row">
//                     <span>🔗</span>
//                     <a href={viewProfile.platform} target="_blank" rel="noreferrer"
//                       style={{ color: "#4f46e5", textDecoration: "none", wordBreak: "break-all" }}>
//                       {viewProfile.platform}
//                     </a>
//                   </div>
...
//                 <button
//                   className="mo-btn mo-chat"
//                   onClick={() => { setViewProfile(null); router.push(`/messages?with=${viewProfile._id}`); }}
//                 >
//                   💬 Open Chat
//                 </button>
...
//                   {status === "pending"  && <div className="cc-badge cc-badge-pend">⏳ Pending</div>}

//                   <div className="cc-top">
//                     {i
//                       ? <img src={i} alt={n} className="cc-av" />
//                       : <div className="cc-avph">{n.charAt(0).toUpperCase()}</div>
//                     }
...
//                         ⏳ Request sent. Waiting for the creator to accept.
//                         View Profile and Chat will unlock once they accept.
//                       </div>
//                     )}

//                     <div className="cc-acts">
//                       {status === "accepted" ? (
//                         // Creator accepted → show View Profile + Chat
//                         <>
//                           <button className="cbtn-profile" onClick={() => setViewProfile(creator)}>
//                             👤 View Profile
//                           </button>
//                           <button
//                             className="cbtn-chat"
...
//                         </button>
//                       ) : (
//                         // No request yet → Connect button
//                         <button
//                           className="cbtn-connect"
//                           onClick={() => handleConnect(id)}
//                           disabled={isConn}
//                         >
//                           {isConn ? "Sending Request..." : "🤝 Connect"}
//                         </button>
//                       )}
//                     </div>
//                   </div>
