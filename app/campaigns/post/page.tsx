...
const SUGGESTED_CATEGORIES = ["fashion","fitness","tech","food","travel","beauty","lifestyle","gaming","education","health","finance","music","sports","art","photography"];
...
const toCanonical = (s: string): string => {
  ...
  const map: Record<string, string> = {
    ...
    "pro":"pro_monthly","pro_plus":"pro_plus_monthly","pro+":"pro_plus_monthly",
    "pro_year":"pro_yearly","pro_plus_year":"pro_plus_yearly",
  };
  return map[v] ?? "free";
};
...
export default function PostCampaignPage() {
  ...
  // ✅ Backend se live tokens fetch karo
  const fetchLiveTokens = async (token: string) => {
    try {
      ...
      if (b !== null) {
        ...
        const stored = localStorage.getItem("cb_user");
        if (stored) setAvailableTokens(Number(JSON.parse(stored).bits ?? 0));
      }
    } catch {
      const stored = localStorage.getItem("cb_user");
      if (stored) setAvailableTokens(Number(JSON.parse(stored).bits ?? 0));
  ...
  const barPercent       = planInfo.tokens > 0 ? Math.min(100, (availableTokens / planInfo.tokens) * 100) : Math.min(100, (availableTokens / 200) * 100);
    ...
    const options = {
      ...
      handler: async (response: any) => {
        ...
        const planDbMap: Record<string, string> = {
          "pro":"pro_monthly","pro_plus":"pro_plus_monthly","pro_year":"pro_yearly","pro_plus_year":"pro_plus_yearly",
        };
        ...
        <div className="up-overlay" onClick={() => setShowUpgradeModal(false)}>
          <div className="up-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="up-modal-sub">
              Campaign post karne ke liye <strong style={{color:"#fff"}}>{TOKENS_PER_CAMPAIGN} tokens</strong> chahiye.<br/>
              Aapke paas abhi <strong style={{color:"#f87171"}}>{availableTokens} tokens</strong> hain.<br/>
              Tokens campaigns, email unlock, Instagram unlock sab mein shared hain.
            </div>
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="up-plan-desc">
                      {plan.tokens.toLocaleString("en-IN")} tokens · ~{Math.floor(plan.tokens / TOKENS_PER_CAMPAIGN)} campaigns possible
                    </div>
                  </div>
            ...
            {isAtLimit ? (
              <div className="pcp-blocked">
                ...
                <p className="pcp-blocked-sub">
                  Campaign post karne ke liye <strong>{TOKENS_PER_CAMPAIGN} tokens</strong> chahiye.<br/>
                  Aapke paas abhi <strong>{availableTokens} tokens</strong> hain.<br/>
                  Tokens email unlock aur Instagram unlock mein bhi use hote hain.
                </p>
              ...
              <form onSubmit={handleSubmit} className="pcp-form">
                <div className="pcp-section">
                  ...
                  {/* Token cost note */}
                  <div style={{display:"flex",alignItems:"center",gap:6,borderRadius:10,padding:"10px 14px",fontSize:13,background: isLow ? "#fffbeb" : "#f8f7ff",border:`1px solid ${isLow ? "#fde68a" : "#e8e5ff"}`,color: isLow ? "#92400e" : "#7c3aed",marginTop:4}}>
                    🪙 This campaign costs <strong style={{margin:"0 3px"}}>{TOKENS_PER_CAMPAIGN} tokens</strong> · You have <strong style={{margin:"0 3px",color: isLow ? "#d97706" : "#4f46e5"}}>{availableTokens}</strong> remaining after this: <strong style={{margin:"0 3px"}}>{Math.max(0, availableTokens - TOKENS_PER_CAMPAIGN)}</strong>
                  </div>
                </div>
                ...
                <div className="pcp-section">
                  ...
                  {/* ✅ CATEGORIES — type + suggest + custom */}
                  <div className="pcp-field">
                    <label className="pcp-label">Categories</label>
                    <div className="cat-wrap">
                      <div className="cat-box" onClick={() => (document.getElementById("cat-input") as HTMLInputElement)?.focus()}>
                  ...
                  <div className="pcp-field" style={{marginBottom:0}}>
                    ...
                    <div className="pcp-chips">
                      {ROLE_OPTIONS.map(r => (
                        <button key={r} type="button" className={`pcp-chip ${formData.roles.includes(r) ? "sel" : ""}`} onClick={() => toggleRole(r)}>{r}</button>
                      ))}
                    </div>
                  </div>
                </div>
...
// // ✅ isSubscribed pe depend mat karo — plan string se check karo
// const getPlanInfo = (planStr: string | null) => {
//   if (!planStr) return PLAN_LIMITS["free"];
//   const canonical = toCanonical(planStr);
//   return PLAN_LIMITS[canonical] ?? PLAN_LIMITS["free"];
// };
...
//     // ✅ Plan string se check karo — isSubscribed pe depend mat karo
//     const plan = parsed.plan || parsed.activePlan || "free";
//     setActivePlan(plan);

//     const token = parsed.token || localStorage.getItem("token");
//     if (token) fetchUsedCampaigns(token, plan);

//     setChecking(false);
//   }, []);

//   const fetchUsedCampaigns = async (token: string, plan: string) => {
//     try {
...
//       // ✅ Paid plan hai toh planActivatedAt se count, warna month start se
//       const isPaid = isPaidPlan(plan);
//       let planStart: Date;
//       if (isPaid && parsed.planActivatedAt) {
//         planStart = new Date(parsed.planActivatedAt);
//       } else {
//         planStart = new Date();
//         planStart.setDate(1);
...
//     const trimmed = cat.trim().toLowerCase();
//     if (!trimmed) return;
//     if (formData.categories.includes(trimmed)) return;
//     setFormData(prev => ({ ...prev, categories: [...prev.categories, trimmed] }));
//     setCategoryInput("");
//     setShowSuggestions(false);
//   };
...
//                 <div
//                   key={plan.id}
//                   className={`up-plan-card ${selectedPlan === plan.id ? "popular-card" : ""}`}
//                   onClick={() => setSelectedPlan(plan.id)}
//                   style={selectedPlan === plan.id ? {borderColor:"rgba(79,70,229,0.7)",background:"rgba(79,70,229,0.12)"} : {}}
//                 >
//                   {plan.popular && <div className="up-popular-tag">POPULAR</div>}
...
//               </div>
//               {!isAtLimit && (
//                 <button className={`pcp-upgrade-btn ${isLow ? "warn-btn" : "normal"}`} onClick={e => { e.stopPropagation(); setShowUpgradeModal(true); }}>
//                   {isSubscribed ? "Change Plan" : "Upgrade ✦"}
//                 </button>
//               )}
//               {isAtLimit && (
//                 <button className="pcp-upgrade-btn danger-btn" onClick={e => { e.stopPropagation(); setShowUpgradeModal(true); }}>
//                   Upgrade!
//                 </button>
//               )}
//             </div> */}
...
//                       )}
//                     </div>
//                     <div className="cat-hint">Press Enter or comma to add · Type anything custom</div>
//                   </div>
...
//   const [showUpgradeModal, setShowUpgradeModal] = useState(false);
//   const [loadingPlan, setLoadingPlan]           = useState<string | null>(null);
//   // ✅ FIXED: moved selectedPlan state here at component level (was inside IIFE — Rules of Hooks violation)
//   const [selectedPlan, setSelectedPlan]         = useState<string>("pro");
