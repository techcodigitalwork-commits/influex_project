...
export default function CreatorProfilePage() {
  ...
  const fetchedIdRef = useRef<string | null>(null); // ← ADD
  ...
  useEffect(() => {
    if (!id) return;                              // ← ADD
    if (fetchedIdRef.current === id) return;      // ← ADD
    fetchedIdRef.current = id;                    // ← ADD
  ...
  const fetchAll = async (tok: string, qAppId: string, qCampaignId: string) => {
    try {
      ...
      try {
        ...
        const pr = await fetch(`${API}/posts/${id}`, {
          headers: { Authorization: `Bearer ${tok}` },
        });
        if (pr.ok) {
          const pd = await pr.json();
          const items = pd?.data || pd?.posts || [];
      ...
      {activeMedia && (
        <div className="pf-modal-overlay" onClick={() => setActiveMedia(null)}>
          ...
          {activeMedia.type === "reel" ? (
            <video className="pf-modal-video" src={activeMedia.url} controls autoPlay playsInline onClick={(e: any) => e.stopPropagation()} />
          ) : (
            <img className="pf-modal-img" src={activeMedia.url} alt={activeMedia.caption || "post"} onClick={(e: any) => e.stopPropagation()} />
          )}
...
//       // ✅ Sirf us creator ki posts fetch karo jiska profile dekh rahe hain
//       try {
//         setPortfolioLoading(true);
//         const pr = await fetch(`${API}/posts/${id}`, {
//           headers: { Authorization: `Bearer ${tok}` },
