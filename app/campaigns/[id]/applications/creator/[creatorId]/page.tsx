...
export default function CreatorProfilePage() {
  ...
  // params keys depend on folder names: [id] = campaignId, [creatorId] = creatorId
  const id         = (params?.creatorId || params?.id) as string;
  ...
  useEffect(() => {
    ...
    const qCampaignId = urlParams.get("campaignId") || (params?.id as string) || "";
    setAppId(qAppId);
  ...
  const fetchProfile = async (tok: string, appId: string|null, campaignId: string|null) => {
    try {
      setLoading(true);
      console.log("🔍 params:", JSON.stringify(params), "| id:", id, "appId:", appId);
      // Fetch profile
