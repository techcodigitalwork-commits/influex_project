...
export default function CampaignApplications() {
  ...
  const fetchData = async (token: string) => {
    try {
      ...
      // ✅ Fetch deals — correct route /deal/my, paymentStatus check
      try {
        const dealsRes  = await fetch(`${API_BASE}/deal/my`, { headers: { Authorization: `Bearer ${token}` } });
        if (dealsRes.ok) {
          const dealsData = await dealsRes.json();
          const deals: any[] = dealsData?.data || dealsData?.deals || [];
          const doneSet = new Set<string>();
          deals.forEach((deal: any) => {
            // ✅ paymentStatus "deposited" ya "released" dono pe unlock karo
            const ps = (deal.paymentStatus || deal.status || "").toLowerCase();
            const isActive = ["deposited", "released", "completed", "done", "active"].includes(ps);
            if (!isActive) return;
            // Match by influencerId
            const infId = typeof deal.influencerId === "object"
              ? (deal.influencerId?._id || deal.influencerId?.id || "")
              : (deal.influencerId || "");
            // Add influencerId to doneSet
            if (infId) doneSet.add(String(infId));
            // Also add applicationId if present
  ...
  return (
    <>
      ...
      {modalProfile && (() => {
        ...
        const modalInfluencerId = String(app?.influencerId?._id || app?.influencer?._id || app?.influencer?.user || app?.userId || "");
        const isDealDone  = dealDoneIds.has(app._id) || dealDoneIds.has(modalInfluencerId);
        ...
        {applications.length === 0 ? (
          ...
          <div className="ap-grid">
            {applications.map(app => {
              ...
              // ✅ Match by app._id OR influencerId
              const appInfluencerId = String(app?.influencerId?._id || app?.influencer?._id || app?.influencer?.user || app?.userId || "");
              const isDealDone = dealDoneIds.has(app._id) || dealDoneIds.has(appInfluencerId);
...
//       // ✅ Fetch deals — correct route /deal/my
//       try {
//         const dealsRes = await fetch(`${API_BASE}/deal/my`, { headers: { Authorization: `Bearer ${token}` } });
//         if (dealsRes.ok) {
//           const dealsData = await dealsRes.json();
...
//           const doneSet = new Set<string>();
//           deals.forEach((deal: any) => {
//             // ✅ paymentStatus === "released" = deal complete
//             const ps = (deal.paymentStatus || deal.status || "").toLowerCase();
//             const isDone = ["released", "completed", "done"].includes(ps);
//             if (isDone) {
//               // Store influencerId so we can match against any app from that creator
//               const infId = typeof deal.influencerId === "object"
//                 ? (deal.influencerId?._id || deal.influencerId?.id || "")
//                 : (deal.influencerId || "");
//               if (infId) doneSet.add(String(infId));
//               if (deal.applicationId) doneSet.add(String(deal.applicationId));
//               if (deal._id) doneSet.add("deal_" + String(deal._id));
//             }
//           });
//           setDealDoneIds(doneSet);
//         }
...
//   // ✅ Check if deal is done for this app — match by influencerId OR applicationId
//   const checkDealDone = (app: any): boolean => {
//     if (dealDoneIds.size === 0) return false;
//     const appId = String(app._id || "");
//     const infId = String(
//       app?.influencerId?._id || app?.influencer?.user ||
//       app?.influencer?._id  || app?.userId || ""
//     );
//     return dealDoneIds.has(appId) || dealDoneIds.has(infId);
//   };
...
//         .ap-card.accepted { border-color: #86efac; }
//         .ap-card.rejected { border-color: #fca5a5; opacity: 0.7; }
//         .ap-card.deal-done { border-color: #4f46e5; background: #fafbff; }
//         .ap-ribbon { position: absolute; top: 14px; right: 14px; padding: 4px 12px; border-radius: 100px; font-size: 11px; font-weight: 700; z-index: 2; }
//         .ap-ribbon.accepted { background: #dcfce7; color: #15803d; border: 1px solid #86efac; }
//         .ap-ribbon.rejected { background: #fee2e2; color: #b91c1c; border: 1px solid #fca5a5; }
//         .ap-ribbon.pending  { background: #eff6ff; color: #2563eb; border: 1px solid #bfdbfe; }
//         .ap-ribbon.deal-done { background: #eef2ff; color: #4f46e5; border: 1px solid #c7d2fe; }
//         .ap-card-top { padding: 14px 16px 10px; display: flex; align-items: center; gap: 12px; }
//         .ap-avatar { width: 52px; height: 52px; border-radius: 14px; object-fit: cover; background: linear-gradient(135deg,#e0e0ff,#f0e0ff); flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: 22px; font-weight: 800; color: #4f46e5; }
//         .ap-avatar img { width: 100%; height: 100%; border-radius: 14px; object-fit: cover; }
//         .ap-name-wrap { flex: 1; min-width: 0; }
...
//         const platform  = getPlatform(app);
//         const note      = app?.note || app?.message || app?.coverLetter || "";
//         const isDealDone = checkDealDone(app);
...
//                 <div className="mo-section">Profile Details</div>
//                 <div className="mo-grid">
//                   <div className="mo-item"><div className="mo-item-label">Followers</div><div className="mo-item-val">{getFollowers(app) || "N/A"}</div></div>
//                   <div className="mo-item"><div className="mo-item-label">Category</div><div className="mo-item-val" style={{ fontSize: 12 }}>{getCategories(app) || "N/A"}</div></div>
//                 </div>
...
//                     {isDealDone
//                       ? <span style={{fontSize:14,fontWeight:700,color:"#111"}}>{phone || "Not provided"}</span>
//                       : <span style={{fontSize:13,color:"#aaa"}}>+91 XXXXXXXXXX</span>}
//                   </div>
//                   <div className="mo-item" style={{display:"flex",alignItems:"center",gap:10}}>
//                     <span>📸</span>
//                     {isDealDone
//                       ? (platform ? <a href={platform} target="_blank" rel="noreferrer" style={{fontSize:13,color:"#4f46e5",fontWeight:600}}>{platform}</a> : <span style={{fontSize:13,color:"#aaa"}}>Not provided</span>)
//                       : <span style={{fontSize:13,color:"#aaa"}}>instagram.com/XXXXXX</span>}
//                   </div>
//                   <div style={{fontSize:11,fontWeight:600,marginTop:8,textAlign:"center",color:isDealDone?"#16a34a":"#f59e0b"}}>
//                     {isDealDone ? "✅ Deal completed — contact unlocked" : "🔒 Complete the deal to unlock contact details"}
//                   </div>
//                 </div>
...
//                 <div className="mo-actions">
//                   {isDealDone ? (
//                     <div className="ap-btn ap-btn-accepted" style={{flex:1}}>✅ Deal Completed</div>
//                   ) : decision === "accepted" ? (
//                     <div className="ap-btn ap-btn-accepted" style={{ flex: 1 }}>✓ Accepted</div>
//                   ) : decision === "rejected" ? (
//                     <div className="ap-btn ap-btn-rejected" style={{ flex: 1 }}>✗ Rejected</div>
...
//             <div className="ap-stat"><span style={{ fontSize: 20 }}>👥</span><div><div className="ap-stat-val">{applications.length}</div><div className="ap-stat-lbl">Total</div></div></div>
//             <div className="ap-stat"><span style={{ fontSize: 20 }}>✅</span><div><div className="ap-stat-val">{Object.values(decidedApps).filter(v => v === "accepted").length}</div><div className="ap-stat-lbl">Accepted</div></div></div>
//             <div className="ap-stat"><span style={{ fontSize: 20 }}>⏳</span><div><div className="ap-stat-val">{applications.filter(a => !decidedApps[a._id] && !checkDealDone(a)).length}</div><div className="ap-stat-lbl">Pending</div></div></div>
//             <div className="ap-stat"><span style={{ fontSize: 20 }}>🤝</span><div><div className="ap-stat-val">{applications.filter(a => checkDealDone(a)).length}</div><div className="ap-stat-lbl">Deals Done</div></div></div>
//           </div>
//         )}
...
//               const decision   = decidedApps[app._id];
//               const initials   = name.slice(0, 2).toUpperCase();
//               const isDealDone = checkDealDone(app); // ✅ use helper

//               return (
//                 <div key={app._id} className={`ap-card ${isDealDone ? "deal-done" : (decision || "")}`}>
//                   {/* ✅ Ribbon — deal done overrides accepted */}
//                   <div className={`ap-ribbon ${isDealDone ? "deal-done" : (decision || "pending")}`}>
//                     {isDealDone ? "🤝 Deal Done" : decision === "accepted" ? "✓ Accepted" : decision === "rejected" ? "✗ Rejected" : "⏳ Pending"}
//                   </div>

//                   <div className="ap-card-top">
//                     <div className="ap-avatar">{img ? <img src={img} alt={name} /> : initials}</div>
//                     <div className="ap-name-wrap">
//                       <div className="ap-name">
//                         {(() => {
//                           const { first, last } = splitName(name);
...
//                   <div className="ap-card-info">
//                     <div className="ap-info-item"><div className="ap-info-label">Followers</div><div className="ap-info-val">{followers || "N/A"}</div></div>
//                     <div className="ap-info-item"><div className="ap-info-label">Category</div><div className="ap-info-val" style={{ fontSize: 12 }}>{categories || "N/A"}</div></div>
//                   </div>
...
//                       {isDealDone
//                         ? <span style={{fontSize:13,fontWeight:700,color:"#111"}}>{phone || "Not provided"}</span>
//                         : <span style={{fontSize:13,fontWeight:600,color:"#aaa"}}>+91 XXXXXXXXXX</span>}
//                     </div>
//                     <div style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:"#f9f9f8",borderRadius:8,border:"1px solid #f0f0f0"}}>
//                       <span style={{fontSize:13}}>📸</span>
//                       {isDealDone
//                         ? (platform ? <a href={platform} target="_blank" rel="noreferrer" style={{fontSize:12,fontWeight:700,color:"#4f46e5",textDecoration:"none"}}>{platform}</a> : <span style={{fontSize:13,color:"#aaa"}}>Not provided</span>)
//                         : <span style={{fontSize:13,fontWeight:600,color:"#aaa"}}>instagram.com/XXXXXX</span>}
//                     </div>
//                     {isDealDone && <div style={{fontSize:10,color:"#16a34a",fontWeight:700,marginTop:5,textAlign:"center"}}>✅ Deal completed — contact unlocked</div>}
//                   </div>
...
//                         👤 View Profile
//                       </button>
//                       {/* ✅ Deal done → no accept/reject, show completed */}
//                       {isDealDone ? (
//                         <div className="ap-btn ap-btn-accepted">✅ Deal Done</div>
//                       ) : decision === "accepted" ? (
//                         <div className="ap-btn ap-btn-accepted">✓ Accepted</div>
//                       ) : decision === "rejected" ? (
//                         <div className="ap-btn ap-btn-rejected">✗ Rejected</div>
...
//                       )}
//                     </div>
//                     {/* Deal button only if accepted and not yet done */}
//                     {decision === "accepted" && !isDealDone && (
//                       <div className="ap-actions-row">
//                         <a href={`/deals/create?campaignId=${id}&creatorId=${app.influencer?._id || app._id}`} className="ap-btn ap-btn-deal">🤝 Deal</a>
//                       </div>
//                     )}
...
//       // Fetch deals to know which are completed
//       try {
//         const dealsRes  = await fetch(`${API_BASE}/deals`, { headers: { Authorization: `Bearer ${token}` } });
//         const dealsData = await dealsRes.json();
//         const deals: any[] = dealsData?.deals || dealsData?.data || [];
//         const doneSet = new Set<string>();
//         deals.forEach((deal: any) => {
...
//                     {decision === "accepted" && (
//                       <div className="ap-actions-row">
//                         <a href={`/dealscontracts/create?campaignId=${id}&creatorId=${app.influencer?._id || app._id}`} className="ap-btn ap-btn-deal">🤝 Deal</a>
//                         {/* <a href={`//create?campaignId=${id}&creatorId=${app.influencer?._id || app._id}`} className="ap-btn ap-btn-contract">📄 Contract</a> */}
//                       </div>
//                     )}
//                   </div>
//                 </div>
