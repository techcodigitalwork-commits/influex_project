...
export default function NotificationsPage() {
  ...
  const getApplicationId = async (applicationId: string, notifId: string): Promise<string> => {
    if (applicationId && applicationId.length === 24) return applicationId;
    ...
    // Find by sender/creatorId
    const senderId = extractMongoId(n.sender);
    const app = apps.find((a: any) =>
      extractMongoId(a.influencerId) === senderId ||
      extractMongoId(a.influencer)   === senderId
    ) || apps[apps.length - 1];
  ...
  const acceptCreator = async (applicationId: string, notifId: string) => {
    try {
      ...
      // Get real applicationId if missing
      const realAppId = await getApplicationId(applicationId, notifId);
      if (!realAppId) throw new Error("Application not found");

      // Try both route patterns
      let res = await fetch(`${API}/application/${realAppId}/decision`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${user.token}` },
        body: JSON.stringify({ decision: "accepted" })
      });
      if (!res.ok) {
        // Fallback: /campaigns/applications/:id/decide
        res = await fetch(`${API}/campaigns/applications/${realAppId}/decide`, {
          method: "PUT",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${user.token}` },
      ...
      saveDecidedStatus(realAppId, "accepted");
      setNotifications(prev => prev.map(n => n._id === notifId ? { ...n, applicationStatus: "accepted", applicationId: realAppId } : n));
      if (selectedProfile?.notifId === notifId) setSelectedProfile((p: any) => ({ ...p, status: "accepted", applicationId: realAppId }));
      // Send notification to creator
      try {
        const notif = notifications.find(x => x._id === notifId);
        const creatorId = extractMongoId(notif?.sender);
        if (creatorId) {
          await safeFetch(`${API}/notification/create`, {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${user.token}` },
            body: JSON.stringify({ userId: creatorId, message: "🎉 Your application has been accepted!", type: "application_accepted", link: "/notification" })
          });
        }
      } catch { /* silent */ }
  ...
  const rejectCreator = async (applicationId: string, notifId: string) => {
    try {
      ...
      saveDecidedStatus(realAppId, "rejected");
