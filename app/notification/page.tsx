...
export default function NotificationsPage() {
  ...
  const fetchNotifications = async (token: string) => {
    try {
      ...
      // ✅ Brand + Creator dono ke liye — sirf unread notifications ka sender name fetch karo
      const unreadNotifs = merged.filter((n: any) => n.read === false && !creatorNames[n._id]);
      if (unreadNotifs.length > 0) {
        const names: Record<string, string> = {};
        await Promise.all(unreadNotifs.map(async (n: any) => {
          const senderId = extractMongoId(n.sender);
          if (!senderId) return;
          if (profileCache[senderId]) {
            names[n._id] = profileCache[senderId]?.name || "";
            return;
          }
          ...
          if (ok && pd) {
            ...
            if (p) { profileCache[senderId] = p; names[n._id] = p.name || ""; }
          }
        }));
                        ...
                        <div style={{ marginTop: "10px" }}>
                          ...
                          {creatorNames[n._id] && (
                            <div style={{ fontSize: 12, color: "#888", marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
                              <span>🏢</span>
                              <span style={{ fontWeight: 600, color: "#555" }}>{creatorNames[n._id]}</span>
