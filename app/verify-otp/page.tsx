...
function VerifyOtpForm() {
  ...
  const handleVerify = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // ── Step 1: emailVerified = true localStorage mein save karo ──
      if (data.token) {
        // Backend ne naya token diya
        localStorage.setItem("token", data.token);

        if (data.user) {
          // Backend ne pura user diya — fresh save karo
          const userData = {
            ...data.user,
            token: data.token,
            hasProfile: false,
          ...
          // Backend ne sirf token diya, user nahi — purana update karo
          const stored = localStorage.getItem("cb_user");
          if (stored) {
            const existing = JSON.parse(stored);
            existing.emailVerified = true;
            existing.token = data.token;
            localStorage.setItem("cb_user", JSON.stringify(existing));
          }
        }
      } else {
        // Backend ne token bhi nahi diya — sirf emailVerified true karo
        const stored = localStorage.getItem("cb_user");
        if (stored) {
          const existing = JSON.parse(stored);
          existing.emailVerified = true;
