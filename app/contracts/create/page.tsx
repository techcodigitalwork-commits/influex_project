...
function CreateContractPageInner() {
  ...
  useEffect(() => {
    ...
    if (campId || creatorId || dealId) {
      setForm(f => ({ ...f, campaignId: campId || f.campaignId, influencerId: creatorId || f.influencerId, dealId: dealId || f.dealId }));
      if (campId) fetchApplications(campId, p.token, creatorId);
    }
  }, []);
  ...
  const fetchApplications = async (campaignId: string, t?: string, preselectedCreatorId?: string) => {
    ...
    const tok = t || token;
    try {
      const res  = await fetch(`${API}/campaigns/${campaignId}/applications`, { headers: { Authorization: `Bearer ${tok}` } });
      const data = await res.json();
      const raw  = Array.isArray(data) ? data : Array.isArray(data.applications) ? data.applications : Array.isArray(data.data) ? data.data : [];

      // Enrich each app with real name from profile API
      const enriched = await Promise.all(raw.map(async (app: any) => {
        let name = app?.influencer?.name || app?.name || "";
        if (name && name !== "Creator") return { ...app, _resolvedName: name };
        const userId = app?.influencerId?._id || app?.influencer?._id || app?.influencer?.user || app?.userId
          || (typeof app?.influencerId === "string" ? app.influencerId : "");
        if (!userId) return { ...app, _resolvedName: name || "Creator" };
        try {
        ...
        return { ...app, _resolvedName: name || "Creator" };
      }));

      setApplications(enriched);

      if (preselectedCreatorId && enriched.length > 0) {
        const found = enriched.find((a: any) =>
          (a.influencer?._id || a.influencer?.user || a._id || a.userId
            || (typeof a.influencerId === "string" ? a.influencerId : a.influencerId?._id)) === preselectedCreatorId
        );
        if (found) setForm(f => ({ ...f, influencerId: preselectedCreatorId, creatorName: found._resolvedName || "" }));
      }
  ...
  const handleCampaignChange = (cid: string) => {
    ...
    if (cid) fetchApplications(cid); else setApplications([]);
  };

  const handleCreatorChange = (cid: string) => {
    const app = applications.find(a =>
      (a.influencer?._id || a.influencer?.user || a.userId
        || (typeof a.influencerId === "string" ? a.influencerId : a.influencerId?._id) || a._id) === cid
    );
    const name = app?._resolvedName || app?.influencer?.name || app?.name || "";
  ...
  const handleSubmit = async (sendNow: boolean) => {
    ...
    try {
      ...
      if (sendNow && newId) {
        try { await fetch(`${API}/contract/${newId}/send`, { method: "POST", headers: { Authorization: `Bearer ${token}` } }); } catch { /* non-fatal */ }
      }
      showToast(sendNow ? "✅ Contract sent to creator!" : "💾 Contract saved as draft!", "success");
      setTimeout(() => { if (newId) router.push(`/contracts/${newId}`); else router.push("/contracts"); }, 900);
    } catch (err: any) {
        ...
        <div className="cc-body">
          <div className="cc-card">
            ...
            {form.campaignId && (
              <div className="cc-field">
                ...
                {loadingApps ? (
                  <div style={{ fontSize: 13, color: "#aaa", padding: "10px 0", display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 14, height: 14, border: "2px solid #e0e0e0", borderTopColor: "#4f46e5", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} /> Loading...
                  </div>
                ) : applications.length === 0 ? (
                  <div style={{ fontSize: 13, color: "#aaa", padding: "10px 0" }}>⚠️ No applications for this campaign</div>
                ) : (
                  <select className="cc-select" value={form.influencerId} onChange={e => handleCreatorChange(e.target.value)}>
                    <option value="">Select creator...</option>
                    {applications.map(a => {
                      const aid  = a.influencer?._id || a.influencer?.user || a.userId
                        || (typeof a.influencerId === "string" ? a.influencerId : a.influencerId?._id) || a._id;
                      const name = a._resolvedName || a.influencer?.name || a.name || "Creator";
                ...
                {form.creatorName && (
                  <div className="cc-creator-card">
                    ...
                    <div><div style={{ fontSize: 14, fontWeight: 700, color: "#111" }}>{form.creatorName}</div><div style={{ fontSize: 12, color: "#9ca3af" }}>Selected creator</div></div>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="cc-card">
            ...
            <div className="cc-preview">
              <div className="cc-preview-header"><div className="cc-preview-logo">Influex</div><div className="cc-preview-badge">AGREEMENT</div></div>
              <div className="cc-preview-title">{form.title || "Contract Title"}</div>
              <div className="cc-preview-parties">
                <div className="cc-preview-party"><div className="cc-preview-party-lbl">Brand (Party A)</div><div className="cc-preview-party-val">Your Brand</div></div>
                <div className="cc-preview-party"><div className="cc-preview-party-lbl">Creator (Party B)</div><div className="cc-preview-party-val">{form.creatorName || "—"}</div></div>
              </div>
              {form.amount && <div style={{ fontSize: 14, fontWeight: 700, color: "#4f46e5", marginBottom: 12 }}>Amount: ₹{Number(form.amount).toLocaleString("en-IN")}</div>}
          ...
          <div className="cc-actions">
            <button className="cc-btn-draft" disabled={submitting} onClick={() => handleSubmit(false)}>{submitting ? "Saving..." : "💾 Save Draft"}</button>
            <button className="cc-btn-send"  disabled={submitting} onClick={() => handleSubmit(true)}>{submitting ? "Sending..." : "📤 Send to Creator →"}</button>
          </div>
        </div>
      </div>
    </>
