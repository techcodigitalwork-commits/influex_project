...
function ContractDetailInner() {
  ...
  useEffect(() => {
    ...
    // Check ?action=sign in URL without useSearchParams
    const action = new URLSearchParams(window.location.search).get("action");
    if (action === "sign") setTimeout(() => setShowSignModal(true), 600);
  }, [id]);
  ...
  if (!contract) return (
    <div style={{ minHeight: "80vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Plus Jakarta Sans,sans-serif" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ fontSize: 18, fontWeight: 700, color: "#111", marginBottom: 12 }}>Contract not found</div>
        <button onClick={() => router.push("/contracts")} style={{ padding: "10px 24px", background: "#4f46e5", color: "#fff", border: "none", borderRadius: 10, cursor: "pointer", fontFamily: "Plus Jakarta Sans,sans-serif", fontSize: 14, fontWeight: 600 }}>
          ← Back to Contracts
        </button>
      </div>
    </div>
  ...
  const canBrandSend   = role === "brand" && (contract.status === "draft" || contract.status === "pending");
  ...
  const statusStyle = () => {
    if (isSigned)                       return { bg: "#f0fdf4", color: "#16a34a", border: "#86efac",  label: "✅ Signed" };
    if (contract.status === "pending")  return { bg: "#fffbeb", color: "#d97706", border: "#fde68a",  label: "⏳ Awaiting Signature" };
    if (contract.status === "rejected") return { bg: "#fff5f5", color: "#dc2626", border: "#fecaca",  label: "❌ Rejected" };
    return                                     { bg: "#f5f5f5", color: "#888",    border: "#e0e0e0",  label: "📝 Draft" };
  };
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .cd-back{background:#f5f5f3;border:none;border-radius:10px;padding:8px 14px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;color:#555;text-decoration:none}
        .cd-back:hover{background:#ebebeb}
        ...
        .cd-doc-badge{display:inline-block;padding:3px 12px;background:rgba(255,255,255,.15);border-radius:100px;font-size:11px;color:rgba(255,255,255,.9);font-weight:600}
        .cd-doc-body{padding:28px 32px}
        .cd-doc-title{font-size:22px;font-weight:800;color:#111;margin-bottom:20px}
        .cd-doc-section{font-size:11px;font-weight:700;color:#aaa;text-transform:uppercase;letter-spacing:.07em;margin-bottom:8px;margin-top:20px}
        ...
        .cd-party-lbl{font-size:10px;color:#bbb;text-transform:uppercase;letter-spacing:.07em;font-weight:600;margin-bottom:6px}
        ...
        .cd-action-title{font-size:13px;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:.07em;margin-bottom:14px}
        .cd-btn-row{display:flex;gap:10px;flex-wrap:wrap}
        .cd-btn{flex:1;min-width:120px;padding:13px;border-radius:12px;font-size:14px;font-weight:700;font-family:inherit;border:none;cursor:pointer;transition:all .2s}
        .cd-btn:disabled{opacity:.5;cursor:not-allowed}
        .cd-btn-sign{background:linear-gradient(135deg,#16a34a,#22c55e);color:#fff;box-shadow:0 4px 14px rgba(34,197,94,.25)}
        .cd-btn-sign:hover:not(:disabled){transform:translateY(-1px)}
        .cd-btn-send{background:linear-gradient(135deg,#4f46e5,#7c3aed);color:#fff;box-shadow:0 4px 14px rgba(79,70,229,.25)}
        ...
        .sm-overlay{position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;animation:fadeIn .2s}
        .sm-box{background:#fff;border-radius:24px;max-width:420px;width:100%;padding:32px;animation:slideUp .25s ease}
        ...
        .sm-label{font-size:12px;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;display:block}
        .sm-input{width:100%;padding:12px 14px;border-radius:10px;border:1.5px solid #ebebeb;font-size:14px;font-family:inherit;outline:none;margin-bottom:12px}
        .sm-input:focus{border-color:#4f46e5}
        .sm-preview{background:#f9f9f8;border-radius:12px;padding:14px 16px;margin-bottom:20px}
        .sm-preview-lbl{font-size:11px;color:#aaa;text-transform:uppercase;letter-spacing:.06em;font-weight:600;margin-bottom:8px}
        ...
        .sm-btn-cancel{flex:1;padding:13px;border-radius:12px;background:#f5f5f3;color:#555;font-size:14px;font-weight:700;font-family:inherit;border:none;cursor:pointer}
        .sm-btn-sign{flex:1;padding:13px;border-radius:12px;background:linear-gradient(135deg,#16a34a,#22c55e);color:#fff;font-size:14px;font-weight:700;font-family:inherit;border:none;cursor:pointer}
        .sm-btn-sign:disabled{opacity:.6;cursor:not-allowed}
        .toast{position:fixed;bottom:24px;left:50%;transform:translateX(-50%);padding:12px 22px;border-radius:12px;font-size:13px;font-weight:600;z-index:99999;animation:toastIn .3s ease;white-space:nowrap;box-shadow:0 4px 20px rgba(0,0,0,.12);font-family:inherit}
      ...
      {showSignModal && (
        <div className="sm-overlay" onClick={e => e.target === e.currentTarget && setShowSignModal(false)}>
          <div className="sm-box">
            ...
            <input className="sm-input" value={signName} onChange={e => setSignName(e.target.value)} placeholder="Type your name to sign" autoFocus />
            ...
            <div className="sm-legal">
              🔒 This digital signature is legally binding. By clicking "Sign Contract" you confirm that you have read and agree to all terms.
            </div>
      ...
      <div className="cd">
        ...
        <div className="cd-header">
          ...
          <div className="cd-status" style={{ background: ss.bg, color: ss.color, border: `1.5px solid ${ss.border}` }}>
            {ss.label}
          </div>
        </div>
        ...
        <div className="cd-body">
          ...
          {(canCreatorSign || canBrandSend) && (
            <div className="cd-actions">
              <div className="cd-action-title">
                {canCreatorSign ? "✍️ Your signature is required" : "📤 Send this contract to the creator"}
              </div>
          ...
          <div className="cd-doc">
            ...
            <div className="cd-doc-body">
              <div className="cd-doc-title">{contract.title || "Contract"}</div>
              ...
              <div className="cd-doc-parties">
                <div className="cd-party">
                  ...
                  <div className="cd-party-name">{contract._brandName || contract.brandName || "Brand"}</div>
                ...
                <div className="cd-party">
                  ...
                  <div className="cd-party-name">{contract._creatorName || contract.creatorName || "Creator"}</div>
              ...
              <div className="cd-meta-row"><span className="cd-meta-lbl">Amount</span><span className="cd-meta-val" style={{ color: "#4f46e5" }}>₹{(contract.amount || 0).toLocaleString("en-IN")}</span></div>
              {(contract.deadline || contract.timeline) && (
                <div className="cd-meta-row">
                  <span className="cd-meta-lbl">Deadline</span>
                  <span className="cd-meta-val">
                    {contract.deadline
                      ? new Date(contract.deadline).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })
                      : contract.timeline}
                  </span>
                </div>
              )}
              ...
              <div className="cd-meta-row">
                <span className="cd-meta-lbl">Created</span>
                <span className="cd-meta-val">{contract.createdAt ? new Date(contract.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" }) : "—"}</span>
              </div>
              ...
              {getDeliverables().length > 0 && (
                <>
                  ...
                  {getDeliverables().map((d: string, i: number) => (
              ...
              <div className="cd-sig-area">
                <div className="cd-sig-box">
                  <div className="cd-sig-line">
                    {brandSigned && <div className="cd-sig-text">{contract.brandSignedName || contract._brandName || "Brand"}</div>}
                  </div>
                ...
                <div className="cd-sig-box">
                  <div className="cd-sig-line">
                    {creatorSigned && <div className="cd-sig-text">{contract.creatorSignedName || contract._creatorName || "Creator"}</div>}
                  </div>
              ...
              {isSigned && (
                <div style={{ marginTop: 20, textAlign: "center", padding: "14px 20px", background: "linear-gradient(135deg,#f0fdf4,#dcfce7)", border: "1.5px solid #86efac", borderRadius: 12, color: "#15803d", fontWeight: 700, fontSize: 14 }}>
...
//                 <div className="cd-party">
//                   <div className="cd-party-lbl">Brand (Party A)</div>
//                   <div className="cd-party-name">{contract.brandName || contract.brand?.name || contract.brandId?.name || "Brand"}</div>
//                   <div className={`cd-party-sig ${brandSigned ? "signed" : "pending"}`}>{brandSigned ? "✓ Signed" : "⏳ Pending"}</div>
//                   {contract.brandSignedAt && <div style={{ fontSize: 11, color: "#aaa", marginTop: 4 }}>{new Date(contract.brandSignedAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</div>}
//                 </div>
//                 <div className="cd-party">
//                   <div className="cd-party-lbl">Creator (Party B)</div>
//                   <div className="cd-party-name">{contract.creatorName || contract.creator?.name || contract.influencerId?.name || "Creator"}</div>
//                   <div className={`cd-party-sig ${creatorSigned ? "signed" : "pending"}`}>{creatorSigned ? "✓ Signed" : "⏳ Pending"}</div>
//                   {contract.creatorSignedAt && <div style={{ fontSize: 11, color: "#aaa", marginTop: 4 }}>{new Date(contract.creatorSignedAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</div>}
//                 </div>
//               </div>
