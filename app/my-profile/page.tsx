...
const SUB_CATEGORIES: Record<string, string[]> = {
  "Influencer":     ["Beauty Influencer", "Tech Influencer", "Fitness Influencer", "Travel Influencer", "Food Influencer"],
  "Photographer":   ["Wedding Photographer", "Event Photographer", "Product Photographer", "Fashion Photographer"],
  "Singer":         ["Wedding Singer", "Club / Bar Singer", "Indie Artist", "Classical Singer"],
  "Model":          ["Fashion Model", "Commercial Model", "Fitness Model", "Instagram Model"],
  "Makeup Artist":  [],
  "Anchor / Host":  [],
};
...
function CropModal({ src, onDone, onCancel }: { src: string; onDone: (blob: Blob) => void; onCancel: () => void }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:"#fff", borderRadius:20, padding:"24px 20px 20px", width:"100%", maxWidth:380, display:"flex", flexDirection:"column", alignItems:"center", gap:16 }}>
        <div style={{ fontSize:16, fontWeight:700, color:"#111", fontFamily:"DM Sans, sans-serif" }}>Crop Photo</div>
        <div style={{ fontSize:12, color:"#aaa", marginTop:-8, fontFamily:"DM Sans, sans-serif" }}>Drag to reposition · Slider to zoom</div>
        <div ref={containerRef} style={{ width:CONTAINER, height:CONTAINER, position:"relative", overflow:"hidden", background:"#f0f0f0", borderRadius:12, cursor:"move", userSelect:"none", display:"flex", alignItems:"center", justifyContent:"center" }}
          onMouseDown={onMouseDown} onTouchStart={onMouseDown}>
          <img ref={imgRef} src={src} alt="crop" onLoad={onImgLoad}
            style={{ width:imgSize.w||"auto", height:imgSize.h||"auto", display:"block", pointerEvents:"none", position:"absolute", left:(CONTAINER-imgSize.w)/2, top:(CONTAINER-imgSize.h)/2 }}
            draggable={false} />
          {imgSize.w > 0 && (
            <svg style={{ position:"absolute", inset:0, pointerEvents:"none" }} width={CONTAINER} height={CONTAINER}>
              <defs>
                <mask id="hole">
                  <rect width={CONTAINER} height={CONTAINER} fill="white"/>
                  <circle cx={circle.x+(CONTAINER-imgSize.w)/2} cy={circle.y+(CONTAINER-imgSize.h)/2} r={circle.r} fill="black"/>
                </mask>
              </defs>
              <rect width={CONTAINER} height={CONTAINER} fill="rgba(0,0,0,0.5)" mask="url(#hole)"/>
              <circle cx={circle.x+(CONTAINER-imgSize.w)/2} cy={circle.y+(CONTAINER-imgSize.h)/2} r={circle.r} fill="none" stroke="rgba(255,255,255,0.8)" strokeWidth={2}/>
            </svg>
          )}
        </div>
        <div style={{ width:"100%", display:"flex", alignItems:"center", gap:10 }}>
          <span style={{ fontSize:12, color:"#aaa" }}>−</span>
          <input type="range" min={minR} max={maxR} step={1} value={circle.r} onChange={onZoom} style={{ flex:1, accentColor:"#4f46e5" }} />
          <span style={{ fontSize:12, color:"#aaa" }}>+</span>
        </div>
        <div style={{ display:"flex", gap:10, width:"100%" }}>
          <button onClick={onCancel} style={{ flex:1, padding:"12px", borderRadius:10, border:"1.5px solid #ebebeb", background:"#fff", color:"#777", fontSize:14, fontWeight:500, fontFamily:"DM Sans, sans-serif", cursor:"pointer" }}>Cancel</button>
          <button onClick={handleCrop} style={{ flex:1, padding:"12px", borderRadius:10, border:"none", background:"#111", color:"#fff", fontSize:14, fontWeight:600, fontFamily:"DM Sans, sans-serif", cursor:"pointer" }}>Apply Crop ✓</button>
        </div>
      </div>
...
export default function SetupProfile() {
      ...
      .then((data) => {
        if (data.success && data.profile) {
          const existingCategories = Array.isArray(data.profile.categories)
            ...
            : data.profile.categories ? [data.profile.categories] : [];

          const existingSubCategories = Array.isArray(data.profile.subCategories)
            ? data.profile.subCategories
            : data.profile.subCategories ? [data.profile.subCategories] : [];
  ...
  const toggleMainCategory = (cat: string) => {
    ...
    const current: string[] = Array.isArray(form.categories) ? form.categories : [];
    const isRemoving = current.includes(lower);
    const nextCategories = isRemoving
      ? current.filter(c => c !== lower)
      : [...current, lower];
    handleChange("categories", nextCategories);

    // Remove sub-categories that belong to this main category when deselected
    if (isRemoving) {
      const removedSubs = (SUB_CATEGORIES[cat] || []).map(s => s.toLowerCase());
      const currentSubs: string[] = Array.isArray(form.subCategories) ? form.subCategories : [];
      handleChange("subCategories", currentSubs.filter(s => !removedSubs.includes(s)));
    }
  };
  ...
  const toggleSubCategory = (sub: string) => {
    ...
    const current: string[] = Array.isArray(form.subCategories) ? form.subCategories : [];
    handleChange(
      "subCategories",
      current.includes(lower) ? current.filter(s => s !== lower) : [...current, lower]
    );
  };

  // ── Brand category toggle (existing) ───────────────────────
  ...
  const handleSubmit = async () => {
    ...
    const subCats: string[] = Array.isArray(form.subCategories) ? form.subCategories : (form.subCategories ? [form.subCategories] : []);

    const payload: any = {
      role:           storedUser.role?.toLowerCase(),
      name:           form.name  || storedUser.name  || "",
      phone:          String(form.phone || ""),
      bio:            form.bio   || "",
      location:       cityValue,
      followers:      String(form.followers || "0"),
      categories:     cats.map(c => c.toLowerCase()),
      subCategories:  subCats.map(s => s.toLowerCase()),
    ...
    try {
      ...
      if (res.ok) {
        const updatedUser = { ...storedUser, hasProfile: true, name: payload.name };
  ...
  const subCats: string[] = Array.isArray(form.subCategories) ? form.subCategories : [];

  // Which main categories are selected AND have sub-options
  const activeMains = MAIN_CATEGORIES.filter(
    m => cats.includes(m.toLowerCase()) && SUB_CATEGORIES[m]?.length > 0
  );
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .pp-chip.sel-sub { background:#7c3aed; border-color:#7c3aed; color:#fff; }
        .pp-sub-section { background:#f8f7ff; border-radius:12px; padding:14px; border:1.5px solid #e8e5ff; }
        .pp-sub-label { font-size:11px; font-weight:600; color:#6d28d9; text-transform:uppercase; letter-spacing:0.06em; margin-bottom:10px; display:flex; align-items:center; gap:6px; }
        ...
        .pp-email-badge { font-size:12px; color:#888; background:#f5f5f3; border-radius:8px; padding:6px 12px; text-align:center; border:1px solid #ebebeb; }
      `}</style>
        ...
        <div className="pp-card">
          ...
          {step === 2 && role !== "BRAND" && (
            <div className="pp-section-gap">
              ...
              {/* ── WHO ARE YOU (Main Category) ── */}
              <div className="pp-field">
                <label className="pp-label">
                  CATEGORIES{" "}
                  <span style={{ color:"#bbb", fontWeight:400, textTransform:"none", letterSpacing:0 }}>
              ...
              {/* ── SUB CATEGORIES — appear when main is selected ── */}
              {activeMains.map((main) => (
                <div className="pp-sub-section" key={main}>
                  <div className="pp-sub-label">
                    ✨ {main} — what type?
                  </div>
                  <div className="pp-chips">
                    {SUB_CATEGORIES[main].map((sub) => (
                      <button
                        key={sub}
                        className={`pp-chip ${subCats.includes(sub.toLowerCase()) ? "sel-sub" : ""}`}
              ...
              <div className="pp-field">
                ...
                <div className="pp-chips">
                  {["Fashion","Beauty","Fitness","Food","Travel","Tech"].map((cat) => (
                    <button key={cat} className={`pp-chip ${cats.includes(cat.toLowerCase()) ? "sel" : ""}`} onClick={() => toggleCategory(cat)}>
                      {cat}
                    </button>
                  ))}
                </div>
...
//     // ✅ Prefill name + email from localStorage (signup data)
//     setForm({
//       name:  storedUser.name  || "",
//       email: storedUser.email || "",
//     });
...
//             ...prev,
//             ...data.profile,
//             // ✅ name: profile ka use karo, fallback localStorage
//             name:         data.profile.name         || storedUser.name  || "",
//             email:        data.profile.email        || storedUser.email || "",
//             city:         data.profile.city         || data.profile.location || "",
//              phone:        data.profile.phone   ,
...
//       });
//       if (res.ok) {
//         // ✅ save name back to localStorage too
//         const updatedUser = { ...storedUser, hasProfile: true, name: payload.name };
//         localStorage.setItem("cb_user", JSON.stringify(updatedUser));
//         router.push(storedUser.role?.toLowerCase() === "brand" ? "/campaigns" : "/discovery");
//       } else {
