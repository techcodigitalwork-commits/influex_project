...
// ─── Canonical plan normalizer ────────────────────────────────────────────────
// Maps ANY plan string → one of: "free" | "pro" | "pro_plus" | "pro_year" | "pro_plus_year"
const normalPlan = (s: string | null | undefined): string => {
  if (!s) return "free";
  const v = s.toLowerCase().trim();
  if (v === "free" || v === "") return "free";

  // plus+yearly variants
  if (
    v === "pro_plus_yearly" || v === "pro_plus_year" ||
    v === "brand_pro_plus_yearly" || v === "influencer_pro_plus_yearly" ||
    v === "pro+year" || v === "proplusyear"
  ) return "pro_plus_year";

  // plus+monthly variants
  if (
    v === "pro_plus_monthly" || v === "pro_plus" || v === "pro+" ||
    v === "brand_pro_plus_monthly" || v === "influencer_pro_plus_monthly" ||
    v === "proplus"
  ) return "pro_plus";

  // pro+yearly variants
  if (
    v === "pro_yearly" || v === "pro_year" ||
    v === "brand_pro_yearly" || v === "influencer_pro_yearly" ||
    v === "proyear"
  ) return "pro_year";

  // pro+monthly variants
  if (
    v === "pro_monthly" || v === "pro" ||
    v === "brand_pro_monthly" || v === "influencer_pro_monthly"
  ) return "pro";

  return "free";
};

// Check if two plan strings refer to the same plan
const samePlan = (a: string | null | undefined, b: string | null | undefined): boolean =>
  normalPlan(a) === normalPlan(b);
...
export default function UpgradePage() {
  ...
  // ─── Init: localStorage fast load → then API sync ────────────────────────
  useEffect(() => {
    ...
    // ✅ Derive isSubscribed from EITHER isSubscribed field OR plan field
    // Backend sometimes only saves plan without isSubscribed: true
    const rawPlan  = parsed.plan || parsed.activePlan || null;
    const canonical = normalPlan(rawPlan);
    const isPaid   = canonical !== "free";
    const hasSub   = parsed.isSubscribed === true || isPaid; // ← KEY FIX
    ...
    // Sync with backend — profile/me for fresh data
    const syncFromBackend = async () => {
      try {
        ...
        const backendUser = data.user || data.profile?.user || {};
        const freshRaw  =
          backendUser?.plan || backendUser?.activePlan ||
          data?.plan || data?.activePlan || rawPlan;
        const freshPlan = normalPlan(freshRaw);
        const freshIsPaid = freshPlan !== "free";
        // ✅ Same logic: isSubscribed OR paid plan → treat as subscribed
        const freshSub  =
          backendUser?.isSubscribed ?? data?.isSubscribed ?? parsed.isSubscribed ?? freshIsPaid;
        const freshBits = backendUser?.bits ?? data?.profile?.bits ?? data?.bits ?? parsed.bits;

        const updated = {
          ...parsed,
          isSubscribed: freshSub || freshIsPaid,
          plan:         freshIsPaid ? freshPlan : (parsed.plan || "free"),
          activePlan:   freshIsPaid ? freshPlan : (parsed.activePlan || "free"),
        ...
        // Fallback: localStorage values already applied above
      }
    };
  ...
  const openRazorpay = (subscriptionId: string, planId: string, planName: string) => {
    const options = {
      ...
      modal: {
        ondismiss: () => { showToast("Payment cancelled.", "error"); setLoadingPlanId(null); },
      },
    };
  ...
  const activateSubscription = async (planId: string, planName: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // normalPlan handles all variant → canonical mapping
        const canonical = normalPlan(planId);

        const matchedPlan = allPlans.find(p => normalPlan(p.id) === canonical);
        const bitsFromPlan = typeof matchedPlan?.tokens === "number" ? matchedPlan.tokens : 0;
        ...
        const updated = {
          ...
          bits:            data.bits ?? data.user?.bits ?? bitsFromPlan ?? parsed.bits,
        };
        localStorage.setItem("cb_user", JSON.stringify(updated));
        setUser(updated);
        setIsSubscribed(true);
        setActivePlan(canonical); // ✅ always canonical — guaranteed match with isThisActive check

        window.dispatchEvent(
          new CustomEvent("plan_updated", {
            detail: { refresh: true, bits: updated.bits, time: Date.now() },
          })
        );
        try {
          const bc = new BroadcastChannel("msg_unread");
          bc.postMessage({ type: "plan_update" });
          bc.close();
        } catch {}
  ...
  // ✅ THE FIX: isThisActive uses normalPlan on both sides — always matches
  const isPlanActive = (cardId: string): boolean => {
    if (!isSubscribed || cardId === "free") return false;
    return normalPlan(cardId) === activePlan;
  };

  const activePlanDisplayName = (() => {
    const matched = allPlans.find(p => p.id !== "free" && normalPlan(p.id) === activePlan);
    return matched?.name || "Pro";
  })();
    ...
    <>
      ...
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .plan-card.active-plan{border-color:rgba(16,185,129,0.6)!important;background:rgba(16,185,129,0.07)!important;transform:scale(1.02);box-shadow:0 0 0 1px rgba(16,185,129,0.3),0 8px 32px rgba(16,185,129,0.15)!important}
        .plan-card.active-plan:hover{border-color:rgba(16,185,129,0.8)!important;box-shadow:0 0 0 1px rgba(16,185,129,0.5),0 12px 40px rgba(16,185,129,0.2)!important}
        ...
        .plan-active-badge{position:absolute;top:-12px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#10b981,#059669);color:#fff;font-size:11px;font-weight:800;padding:5px 16px;border-radius:100px;white-space:nowrap;box-shadow:0 2px 12px rgba(16,185,129,0.4);letter-spacing:0.03em}
        ...
        .plan-cta.active-cta{background:linear-gradient(135deg,rgba(16,185,129,0.15),rgba(16,185,129,0.08));color:#34d399;border:1px solid rgba(16,185,129,0.4);cursor:default;font-weight:800}
        ...
        {isSubscribed && activePlan !== "free" && (
          <div className="up-active-banner">
            <div className="up-active-inner">
              ...
              <span className="up-active-badge">✓ {activePlanDisplayName} Active</span>
            </div>
          </div>
        )}
            ...
            <>
              ...
              <div className="plans-grid">
                {plans.map((plan: any, i: number) => {
                  const isThisActive = isPlanActive(plan.id); // ✅ uses normalPlan — always correct
                  const isLoading    = loadingPlanId === plan.id;

                  return (
                    <div
                      key={plan.id}
                      className={`plan-card ${isThisActive ? "active-plan" : (i === 1 ? "featured" : "")}`}
                    >
                      {/* Badge */}
                      {isThisActive
                      ...
                      <div style={{ marginTop: (plan.badge || isThisActive) ? 20 : 0 }}>
                        ...
                        <div className="plan-price-row">
                          ...
                          <span className="plan-price">
                            {fmtPrice(plan.price)}<span>{plan.period}</span>
                          </span>
                        </div>
                        <div className="plan-pills">
                          ...
                          {role === "brand" && (
                            <span className="plan-pill camp">📢 {plan.campaigns} campaigns</span>
                          )}
                          {role === "creator" && (
...
//   "pro_monthly": "pro", "pro_plus_monthly": "pro_plus",
//   "pro_yearly": "pro_year", "pro_plus_yearly": "pro_plus_year",
//   "pro": "pro", "pro_plus": "pro_plus",
//   "pro_year": "pro_year", "pro_plus_year": "pro_plus_year",
//   "brand_pro_monthly": "pro", "brand_pro_plus_monthly": "pro_plus",
//   "brand_pro_yearly": "pro_year", "brand_pro_plus_yearly": "pro_plus_year",
//   "influencer_pro_monthly": "pro", "influencer_pro_plus_monthly": "pro_plus",
//   "influencer_pro_yearly": "pro_year", "influencer_pro_plus_yearly": "pro_plus_year",
// };

// const toC = (s: string): string => {
//   if (!s) return "";
//   const v = s.toLowerCase().trim();
//   if (dbPlanMap[v]) return dbPlanMap[v];
//   if (v === "pro+" || v === "proplus") return "pro_plus";
//   if (v === "pro+year" || v === "proplusyear") return "pro_plus_year";
//   if (v === "proyear") return "pro_year";
//   return v;
// };
...
//   const getFreshToken = (): string => {
//     try {
//       return JSON.parse(localStorage.getItem("cb_user") || "{}").token
//         || localStorage.getItem("token") || "";
...
//       });
//     } catch {}
//     // verify ho ya na ho — activate hamesha call karo
//     await activateSubscription(planId, planName);
//   };

//   const activateSubscription = async (planId: string, planName: string) => {
//     // ✅ FIX 1: React closure issue — hamesha fresh token lo localStorage se
//     const freshToken = getFreshToken();
...
//         // ✅ FIX 2: frontend planId → DB canonical name
...
//         // ✅ FIX 3: bits bhi save karo plan ke hisaab se
//         const matchedPlan = allPlans.find(p => p.id === planId);
//         const bitsNum = typeof matchedPlan?.tokens === "number" ? matchedPlan.tokens : 0;

//         // ✅ FIX 4: isSubscribed: true + sab fields properly save
//         const updated = {
//           ...parsed,
//           isSubscribed: true,
...
//           activePlanName: planName,
//           planActivatedAt: new Date().toISOString(),
//           // bits: bitsNum > 0 ? bitsNum : parsed.bits,
//           bits: data.bits ?? data.user?.bits ?? parsed.bits,
...
//         // dusre tabs ke liye BroadcastChannel
//         try {
//           const bc = new BroadcastChannel("msg_unread");
//           bc.postMessage({ type: "plan_update" });
//           bc.close();
...
//         /* ✅ Active plan card — green glow */
//         .plan-card.active-plan{
//           border-color:rgba(16,185,129,0.6) !important;
//           background:rgba(16,185,129,0.07) !important;
//           transform:scale(1.02);
//           box-shadow:0 0 0 1px rgba(16,185,129,0.3), 0 8px 32px rgba(16,185,129,0.15) !important;
//         }
//         .plan-card.active-plan:hover{
//           border-color:rgba(16,185,129,0.8) !important;
//           box-shadow:0 0 0 1px rgba(16,185,129,0.5), 0 12px 40px rgba(16,185,129,0.2) !important;
//         }

//         .plan-badge{position:absolute;top:-12px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#4f46e5,#7c3aed);color:#fff;font-size:11px;font-weight:800;padding:4px 14px;border-radius:100px;letter-spacing:0.05em;text-transform:uppercase;white-space:nowrap}
//         .plan-badge.best{background:linear-gradient(135deg,#7c3aed,#ec4899)}

//         /* ✅ Active plan badge — green */
//         .plan-active-badge{
//           position:absolute;top:-12px;left:50%;transform:translateX(-50%);
//           background:linear-gradient(135deg,#10b981,#059669);
//           color:#fff;font-size:11px;font-weight:800;padding:5px 16px;
...
//           const displayName = matched?.name || "Pro";
//           return (
//             <div className="up-active-banner">
//               <div className="up-active-inner">
//                 <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
...
//                       className={`plan-card ${isThisActive ? "active-plan" : (i === 1 ? "featured" : "")}`}
//                     >
//                       {/* ✅ Active plan — green badge, else normal badge */}
//                       {isThisActive
//                         ? <div className="plan-active-badge">✓ Your Current Plan</div>
//                         : plan.badge
//                           ? <div className={`plan-badge ${i === 2 ? "best" : ""}`}>{plan.badge}</div>
...
// // ✅ DB plan name → frontend plan ID mapping
// const dbPlanMap: Record<string, string> = {
//   "pro_monthly": "pro",
//   "pro_plus_monthly": "pro_plus",
//   "pro_yearly": "pro_year",
//   "pro_plus_yearly": "pro_plus_year",
//   "pro": "pro",
//   "pro_plus": "pro_plus",
//   "pro_year": "pro_year",
...
//   // ✅ FIXED useEffect — DB plan names properly mapped, fetchActivePlan removed (route doesn't exist)
//   useEffect(() => {
//     const stored = localStorage.getItem("cb_user");
//     if (!stored) return;
...
//     if (parsed.isSubscribed === true && rawPlan) {
//       const mappedPlan = toC(rawPlan); // "pro_monthly" → "pro", etc.
//       setIsSubscribed(true);
//       setActivePlan(mappedPlan);

//       // ✅ Update localStorage with normalized plan ID so it's correct next time too
//       if (mappedPlan !== rawPlan) {
//         const updated = { ...parsed, activePlan: mappedPlan };
//         localStorage.setItem("cb_user", JSON.stringify(updated));
//       }
//     }
//   }, []);
...
//               <div className="plans-grid">
//                 {plans.map((plan: any, i: number) => {
//                   // ✅ toC now handles DB plan names like "pro_monthly"
//                   const isThisActive = isSubscribed && plan.id !== "free" && !!activePlan && toC(activePlan) === toC(plan.id);
//                   const isLoading = loadingPlanId === plan.id;
//                   return (
