...
function DealDetailPageInner() {
  ...
  const fetchDeal = async (t: string) => {
    try {
      ...
      // ✅ localStorage se paymentStatus restore karo
      ...
      // ✅ Escrow separately fetch karo — backend mein alag collection hai
      try {
      ...
      const rzp = new (window as any).Razorpay({
        ...
        handler: async (response: any) => {
          try {
            ...
            // ✅ localStorage mein save karo taaki reload pe bhi rahe
            try {
  ...
  // ✅ Brand approves — dealId se kaam karo, deliverableId ki zaroorat nahi
  const handleApprove = async () => {
    if (!confirm("Approve work and release payment to creator?")) return;
    setActionLoading("approve");
    try {
      // ✅ Confirmed working: POST /api/deal/approve with dealId
  ...
  // ✅ isWorkSubmitted — workStatus field se check karo (backend confirmed)
  ...
  const getStep = (): number => {
    ...
    if (deal.status === "pending" || deal.paymentStatus === "pending") return 1;
    return 1;
  };
      ...
      <div className="dd">
        ...
        <div className="dd-body">
          ...
          <div className="dd-card">
            ...
            {/* ✅ Brand: approve work — workStatus === "submitted" hone pe dikhega */}
            {isBrand && isEscrowFunded && isWorkSubmitted && !isCompleted && (
              <>
...
//       console.log("workStatus:", d.workStatus);
//       console.log("brandId:", d.brandId);
//       console.log("influencerId:", d.influencerId);
//       console.log("escrow in deal:", d.escrow);
//       console.log("full deal:", JSON.stringify(d, null, 2));
//     } catch(e:any) {
//       showToast(e.message || "Failed to load deal", "error");
//     } finally {
//       setLoading(false);
...
//   // ── FLOW STATE FLAGS ─────────────────────────────────────
//   // isEscrowFunded: check every possible indicator from backend
...
//   );
//   const isWorkSubmitted = !!(
//     deal?.workStatus === "submitted"     ||
...
//     setActionLoading("approve");
//     try {
//       // Try with dealId first (most backends accept this)
//       let res = await fetch(`${API}/payment/approve-deliverable`, {
//         method:  "POST",
//         headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
//         body:    JSON.stringify({ dealId: id }),
//       });
//       // Fallback: try with deliverableId if present
//       if (!res.ok && (deal?.deliverableId || deal?.submittedWork?._id)) {
//         const deliverableId = deal?.deliverableId?._id || deal?.deliverableId || deal?.submittedWork?._id;
//         res = await fetch(`${API}/payment/approve-deliverable`, {
//           method:  "POST",
//           headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
//           body:    JSON.stringify({ deliverableId, dealId: id }),
//         });
//       }
//       const data = await res.json();
//       if (!res.ok) throw new Error(data.message || "Approval failed");
