...
export default function DealsPage() {
  ...
  const enrichDeals = async (list: any[], t: string) => {
    const enriched = await Promise.all(list.map(async (deal: any) => {
      const brandUserId   = typeof deal.brandId      === "string" ? deal.brandId      : deal.brandId?._id      || "";
      const creatorUserId = typeof deal.influencerId === "string" ? deal.influencerId : deal.influencerId?._id || "";
      let brandName   = deal.brandName   || deal.brand?.name   || deal.brandId?.name   || "";
      let creatorName = deal.creatorName || deal.creator?.name || deal.influencer?.name || deal.influencerId?.name || "";
      ...
      return { ...deal, _brandName: brandName || "Brand", _creatorName: creatorName || "Creator" };
    }));
    return enriched;
  };
  ...
  const fetchDeals = async (t: string) => {
    try {
      ...
      let list: any[] = Array.isArray(data) ? data : (data.deals || data.data || []);
      list.sort((a: any, b: any) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());
      const enriched = await enrichDeals(list, t);
      setDeals(enriched);
    } catch { setDeals([]); }
  ...
  const fetchCampaignDeals = async (campaignId: string) => {
    ...
    try {
      ...
      let list: any[] = Array.isArray(data) ? data : (data.deals || data.data || []);
      list.sort((a: any, b: any) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());
      const enriched = await enrichDeals(list, token);
      setDeals(enriched);
    } catch { setDeals([]); }
    finally { setLoading(false); }
  };
  ...
  const handleDeposit = async (deal: any) => {
    ...
    try {
      ...
      if (data.order?.id) {
        ...
        showToast("💰 Payment deposited!", "success");
        fetchDeals(token);
      }
    } catch (err: any) { showToast(err.message || "Deposit failed", "error"); }
    finally { setActioning(null); }
  };

  const handleApprove = async (deal: any) => {
    if (!confirm("Approve work and release payment to creator?")) return;
    setActioning(deal._id + "_approve");
    try {
      // ✅ backend needs deliverableId
      const deliverableId = deal.deliverable?._id;
      if (!deliverableId) {
        showToast("Deliverable not found. Open deal to approve.", "error");
        setActioning(null);
        return;
      }
      ...
      fetchDeals(token);
    } catch (err: any) { showToast(err.message || "Approve failed", "error"); }
    finally { setActioning(null); }
  };

  // ✅ Helper — use paymentStatus since server doesn't return "status" field
  const getDealStatus = (deal: any) => {
    if (deal.paymentStatus === "released") return "completed";
    if (["paid","deposited"].includes(deal.paymentStatus)) return "active";
    return "pending";
  };
  ...
  const statusColor: Record<string, string> = {
    pending: "#d97706", active: "#4f46e5", completed: "#16a34a",
  };
  const statusBg: Record<string, string> = {
    pending: "#fffbeb", active: "#eef2ff", completed: "#f0fdf4",
  };
  const statusBorder: Record<string, string> = {
    pending: "#fde68a", active: "#c7d2fe", completed: "#86efac",
  };
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .dl-create-btn{padding:10px 20px;background:linear-gradient(135deg,#4f46e5,#6366f1);color:#fff;border-radius:12px;font-size:13px;font-weight:700;text-decoration:none;display:inline-flex;align-items:center;gap:6px;font-family:'Plus Jakarta Sans',sans-serif;transition:all .2s;box-shadow:0 2px 10px rgba(79,70,229,.3)}
        .dl-create-btn:hover{transform:translateY(-1px);box-shadow:0 4px 16px rgba(79,70,229,.4)}
        .dl-flow{display:flex;align-items:center;padding:20px 32px 0;flex-wrap:wrap;overflow-x:auto;gap:0}
        ...
        @media(max-width:600px){.dl-stats{padding:12px 16px 0;gap:8px}}
        ...
        .dl-card{background:#fff;border-radius:18px;border:1.5px solid #efefef;padding:22px;transition:all .22s;cursor:pointer;animation:fadeUp .3s ease both}
        .dl-card:hover{border-color:#d4d0f7;box-shadow:0 8px 32px rgba(79,70,229,.08);transform:translateY(-2px)}
        ...
        .dl-status{padding:4px 12px;border-radius:100px;font-size:11px;font-weight:700;flex-shrink:0;text-transform:capitalize}
        ...
        .dl-escrow-fill{height:100%;border-radius:100px;background:linear-gradient(90deg,#4f46e5,#7c3aed);transition:width .5s ease}
        ...
        .dl-party{flex:1;background:#f9f9f8;border-radius:10px;padding:8px 10px;border:1px solid #f0f0f0;font-size:12px;min-width:0}
        .dl-party-lbl{color:#bbb;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px}
        .dl-party-val{font-weight:700;color:#111;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .dl-actions{display:flex;gap:8px}
        .dl-btn{flex:1;padding:10px;border-radius:10px;font-size:12px;font-weight:700;font-family:'Plus Jakarta Sans',sans-serif;border:none;cursor:pointer;transition:all .2s;text-align:center;text-decoration:none;display:flex;align-items:center;justify-content:center;gap:4px;white-space:nowrap}
        .dl-btn-view{background:#eef2ff;color:#4f46e5}
        .dl-btn-view:hover{background:#e0e7ff}
        .dl-btn-deposit{background:linear-gradient(135deg,#4f46e5,#7c3aed);color:#fff;box-shadow:0 2px 8px rgba(79,70,229,.25)}
        .dl-btn-deposit:hover:not(:disabled){transform:translateY(-1px)}
        .dl-btn-deposit:disabled{opacity:.6;cursor:not-allowed}
        .dl-btn-approve{background:linear-gradient(135deg,#16a34a,#22c55e);color:#fff}
        .dl-btn-approve:hover:not(:disabled){transform:translateY(-1px)}
        .dl-btn-approve:disabled{opacity:.6;cursor:not-allowed}
      ...
      <div className="dl">
        ...
        {deals.length > 0 && !loading && (
          <div className="dl-stats">
            ...
            <div className="dl-stat"><span style={{fontSize:18}}>⏳</span><div><div className="dl-stat-val">{deals.filter(d => getDealStatus(d) === "pending").length}</div><div className="dl-stat-lbl">Pending</div></div></div>
            <div className="dl-stat"><span style={{fontSize:18}}>⚡</span><div><div className="dl-stat-val">{deals.filter(d => getDealStatus(d) === "active").length}</div><div className="dl-stat-lbl">Active</div></div></div>
            <div className="dl-stat"><span style={{fontSize:18}}>✅</span><div><div className="dl-stat-val">{deals.filter(d => getDealStatus(d) === "completed").length}</div><div className="dl-stat-lbl">Done</div></div></div>
            <div className="dl-stat"><span style={{fontSize:18}}>💰</span><div><div className="dl-stat-val">₹{deals.reduce((s,d)=>s+(d.amount||0),0).toLocaleString("en-IN")}</div><div className="dl-stat-lbl">Total Value</div></div></div>
          </div>
        )}
          ...
          deals.length === 0 ? (
            ...
            <div className="dl-grid">
              {deals.map((deal, idx) => {
                ...
                // ✅ escrow bar progress
                const escrowPct = isCompleted(deal) ? 100 : isEscrowFunded(deal) ? 60 : 0;
                ...
                return (
                  <div key={deal._id} className="dl-card" style={{animationDelay:`${idx*0.06}s`}}
                    ...
                    <div className="dl-card-top">
                      <div>
                        <div className="dl-card-title">{deal.title || deal.campaignId?.title || deal.campaignTitle || "Deal"}</div>
                        <div className="dl-card-sub">#{deal._id?.slice(-6)}</div>
                      </div>
                      {/* ✅ Use paymentStatus for badge */}
                      <div className="dl-status" style={{
                        background: statusBg[dealStatus],
                        color:      statusColor[dealStatus],
                        border:     `1px solid ${statusBorder[dealStatus]}`,
                      }}>{dealStatus}</div>
                    </div>
                    ...
                    <div className="dl-amount-row">
                      <div>
                        ...
                        <div className="dl-amount">₹{(deal.amount || 0).toLocaleString("en-IN")}</div>
                      </div>
                      <div style={{textAlign:"right"}}>
                        <div className="dl-amount-lbl">Escrow</div>
                        {/* ✅ Use paymentStatus */}
                        <div style={{fontSize:14,fontWeight:700,color:isEscrowFunded(deal)?"#16a34a":"#f59e0b"}}>
                          {isEscrowFunded(deal) ? "✓ Deposited" : "⏳ Pending"}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="dl-parties">
                      <div className="dl-party">
                        ...
                        <div className="dl-party-val">{deal._brandName}</div>
                      </div>
                      <div className="dl-party">
                        <div className="dl-party-lbl">Creator</div>
                        <div className="dl-party-val">{deal._creatorName}</div>
                      </div>
                    </div>
                    ...
                    <div className="dl-actions" onClick={e => e.stopPropagation()}>
                      ...
                      {/* ✅ Deposit: only if escrow not funded */}
                      {role === "brand" && !isEscrowFunded(deal) && (
                      ...
                      {/* ✅ Approve: workStatus=submitted + deliverable exists + not completed */}
                      {role === "brand" && isWorkSubmitted(deal) && !isCompleted(deal) && (
...
//   // Enrich deals with real brand/creator names from profile API
//   const enrichDeals = async (list: any[], t: string) => {
//     const enriched = await Promise.all(list.map(async (deal: any) => {
//       const brandUserId   = typeof deal.brandId      === "string" ? deal.brandId      : deal.brandId?._id      || "";
//       const creatorUserId = typeof deal.influencerId === "string" ? deal.influencerId : deal.influencerId?._id || "";
...
//   const statusColor: Record<string, string> = {
//     pending: "#f59e0b", active: "#4f46e5", completed: "#16a34a", cancelled: "#ef4444", disputed: "#dc2626",
//   };
//   const statusBg: Record<string, string> = {
//     pending: "#fffbeb", active: "#eef2ff", completed: "#f0fdf4", cancelled: "#fff5f5", disputed: "#fff1f1",
//   };
